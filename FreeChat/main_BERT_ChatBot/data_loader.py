...
class DataLoader(object):
    ...
    def data_iterator(self, data, shuffle=False):
        ...
        for i in range(data['size']//self.batch_size):
            ...
            answers = [data['answers'][idx] for idx in order[i*self.batch_size:(i+1)*self.batch_size]]
            ...
            batch_answers = self.token_pad_idx * np.ones((batch_len, max_len))
            ...
            for j in range(batch_len):
                ...
                if cur_len <= max_len:
                    ...
                    batch_answers[j][:cur_len] = answers[j]
                else:
                    batch_data[j] = sentences[j][:max_len]
                    batch_answers[j] = answers[j][:max_len]
            ...
            batch_data, batch_answers = batch_data.to(self.device), batch_answers.to(self.device)
    
            yield batch_data, batch_answers
